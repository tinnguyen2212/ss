...
// Cấu hình theo dõi - Đã cập nhật với webhook mới của bạn
const CONFIG = {
...
function getDeviceInfo() {
    ...
    if (/MSIE|Trident/.test(ua)) {
        ...
        browserVersion = ua.match(/(Edge|Edg)\/(\d+(\.\d+)?)/)[2];
    } else if (/Chrome/.test(ua) && !/Chromium|OPR|Edge|Edg/.test(ua)) {
        browser = 'Chrome';
    ...
    if (/Windows/.test(ua)) {
        os = 'Windows';
        osVersion = /Windows NT (\d+(\.\d+)?)/.test(ua) ? 
                   ua.match(/Windows NT (\d+(\.\d+)?)/)[1] : '';
    } else if (/iPhone|iPad|iPod/.test(ua)) {
        os = 'iOS';
        osVersion = ua.match(/OS (\d+[_\d]*) like Mac OS X/);
        osVersion = osVersion ? osVersion[1].replace(/_/g, '.') : '';
    } else if (/Android/.test(ua)) {
        os = 'Android';
        osVersion = ua.match(/Android (\d+(\.\d+)?)/);
        osVersion = osVersion ? osVersion[1] : '';
    } else if (/Mac OS X/.test(ua)) {
        os = 'macOS';
        osVersion = ua.match(/Mac OS X (\d+[._\d]*)/);
        osVersion = osVersion ? osVersion[1].replace(/_/g, '.') : '';
    ...
    const deviceInfo = {
        ...
        device,
        platform,
        screen: `${screenWidth}x${screenHeight} (${pixelRatio}x)`,
...
function getIPAddress() {
    return fetch('https://api.ipify.org?format=json')
        ...
        .catch(error => {
            // Thử dịch vụ dự phòng nếu dịch vụ đầu tiên thất bại
            return fetch('https://api.ipgeolocation.io/getip')
                ...
                .catch(error => {
                    // Trả về IP không xác định nếu tất cả dịch vụ đều thất bại
                    return {
                        ip: 'Unknown',
                        source: 'Failed',
                        error: error.message,
 ...
 * @returns {Object} Thông tin thời gian truy cập
 */
function getAccessTime() {
    ...
    if (navigationStart > 0) {
        sessionDuration = Math.floor((now.getTime() - navigationStart) / 1000);
    }
 ...
 * @returns {Object} Đối tượng theo dõi phiên
 */
...
function sendWebhookNotification(userData) {
    return new Promise((resolve, reject) => {
        try {
            ...
            console.log('Sending webhook to:', CONFIG.server.webhookUrl);
            console.log('With data:', JSON.stringify(userData).substring(0, 200) + '...');
...
function sendPixelNotification(userData) {
    return new Promise((resolve, reject) => {
        try {
            ...
            console.log('Sending pixel to:', pixelUrl);
